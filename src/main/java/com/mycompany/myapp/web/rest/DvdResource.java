...
public class DvdResource {
    ...
    private static final String ENTITY_NAME = "jhipsterSampleApplicationDvd";
     ...
     */
    @GetMapping("/dvds")
    public ResponseEntity<List<Dvd>> getAllDvds(Pageable pageable) {
    ...
    /**
     * {@code SEARCH  /_search/dvds?query=:query} : search for the dvd corresponding
     * to the query.
     *
     * @param query the query of the dvd search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/dvds")
    public ResponseEntity<List<Dvd>> searchDvds(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Dvds for query {}", query);
        Page<Dvd> page = dvdService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
